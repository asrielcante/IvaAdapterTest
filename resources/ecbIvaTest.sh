...
validaSalidas(){
	...
	if [ "$nombreA" == "CFDPTCARTER" ] || [ "$nombreA" == "CFDPTSOFOMC" ]  ; then
		...
		if [ ! -e "${pathECBSalida}${nombreA}${fechaA}ORIGINAL_CARTER_${timeStampA}.TXT" ] ; then
			echo "Error:falta archivo de salida ORIGINAL_CARTER_ para ${nombreA}${fechaA}, se detiene el proceso";
			exit -1
		fi
	else
		if [ "$nombreA" == "CFDLMPAMPAS" ] || [ "$nombreA" == "CFDLMPAMPAA" ]  ; then
			...
			if [ ! -e "${pathECBSalida}${nombreA}${fechaA}ORIGINAL_PAMPA_${timeStampA}.TXT" ] ; then
				echo "Error:falta archivo de salida ORIGINAL_PAMPA_ para ${nombreA}${fechaA}, se detiene el proceso";
				exit -1
			fi
		fi
		...
		if [ ! -e "${pathECBSalida}${nombreA}${fechaA}_CONTROL_${timeStampA}.TXT" ] ; then
			echo "Error:falta archivo de salida _CONTROL_ para ${nombreA}${fechaA}, se detiene el proceso";
			exit -1
		fi
		#valida salida original iva
		if [ ! -e "${pathECBSalida}${nombreA}${fechaA}ORIGINAL_IVA_${timeStampA}.TXT" ] ; then
			echo "Error:falta archivo de salida ORIGINAL_IVA_ para ${nombreA}${fechaA}, se detiene el proceso";
			exit -1
		fi
	fi
}
...
then
	echo "ERROR: Falta alguno de los archivos de catalogos"
	exit -1
fi
